...
bool UartTest::execute_class_api_case(){
	...
	print_case_message("This is a UART loopback test, the RX/TX pins need to be shorted");
...
bool UartTest::execute_uart_api_case(u32 port){
	...
	enum Io::peripheral_ports peripheral_port
			= Io::get_peripheral_port(
				Io::peripheral_function_uart, port
				);
	TEST_THIS_ASSERT(
				bool,
				Io::lock_peripheral_pins(peripheral_port) < 0,
				false);
	...
	{
		IoFunctionLock function_lock(peripheral_port);
		Uart uart(port);
		...
		do {
			TEST_THIS_ASSERT(bool, uart.write(outgoing) == outgoing.length_signed(), true);
			wait(Milliseconds(50));
			TEST_THIS_EXPECT(bool, uart.read(incoming) == outgoing.length_signed(), true);

			if( uart.return_value() > 0 ){
				String incoming_string(incoming.to_const_char(), String::Length(uart.return_value()));
